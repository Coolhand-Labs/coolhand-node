...
- **Fixed a regression in the #115 tee fix that could let an intercepted response buffer without bound.** `hostIsReading()` can't distinguish "a host callback will never read the response" from "hasn't read it *yet* because it's still inside an `await` before attaching a listener" — during that window, backpressure never engaged and the tee's internal `PassThrough` buffered the entire response with no cap, the exact unbounded-memory-growth scenario the original fix existed to prevent. A new hard byte ceiling (`MAX_TEE_BUFFERED_BYTES`, 50 MB by default) now bounds growth during that window by destroying the tee rather than pausing the real response — preserving the original fix's guarantee that an unread tee must never stall the interceptor's own capture.
