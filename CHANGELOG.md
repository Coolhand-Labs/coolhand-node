...
### ✨ New Features
- **GitHub Models support** — `models.inference.ai.azure.com` (Microsoft's inference endpoint used by GitHub Copilot) is now in the default intercept list. Calls made via the OpenAI SDK pointed at this endpoint are captured automatically without any `intercept_addresses` configuration.
