...
### ✨ New Features
- **OpenCode support** — `opencode.ai` (the OpenCode Zen model gateway) is now a built-in monitored provider, along with `api.opencode.ai` (an observed misconfigured host that doesn't correspond to any real OpenCode endpoint, captured so a misconfigured integration shows up as a failed/unmatched log instead of vanishing with no trace). Redacts `authorization` and `x-api-key` headers. Works in both Node.js (file-loaded patterns) and Edge runtimes (Cloudflare Workers, Vercel Edge). ([#187](https://github.com/Coolhand-Labs/coolhand-node/issues/187))
...
### 🧹 Internal
- **`paginationFromHeaders` moved from `LoggingService` to `BaseService`**, now shared by `searchLogs` and `searchTemplates` per `CLAUDE.md`'s DRY guidance ("once a second service needs the same helper, promote it to `BaseService`"). Pure move — no behavior change, and it stays `protected`, so the public API surface is unchanged.
- **New opt-in live test suite (`npm run test:live`, `jest.live.config.cjs`, `test/live/*.live.ts`)** — exercises the template methods against a real Coolhand server with no mocking, driven by `COOLHAND_LIVE_BASE_URL`/`COOLHAND_LIVE_API_KEY`. Deliberately excluded from `npm test` rather than conditionally skipped inside it, so CI (which has neither a server nor a private key) stays green without any test being marked skipped. Every request it makes is read-only.

### 🔒 Security
- **`RequestMonitoringService.ts` no longer logs the raw, unsanitized request URL.** This is distinct from #113, which fixed the *stored* `callData.url` — the stored value was already correctly sanitized via `sanitizeURL()`, but two separate `console.log` call sites in `RequestMonitoringService.ts` (unlike their already-correct counterparts in `global-monitor.ts`) still printed the raw local `url`/`options` instead: the `📞 Starting API call`/`📞 Starting FETCH call` logs (`interceptRequest`, `interceptFetch`) used the raw pre-sanitization `url` variable rather than `callData.url`, and `debugRequest` (invoked from every patched entry point when `debug: true`) assigned the *entire* raw URL string — path, query string and all — to a variable it labeled `hostname` and printed verbatim whenever `options` was passed as a plain string or object with a `.url` field (the shape `fetch()` interception uses). Either site would print a live API key to stdout for any provider that puts one in the query string, e.g. Google's `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=AIza...`. Both `console.log` sites now use `callData.url`, and `debugRequest` now parses out just the hostname via `new URL(...).hostname`, matching the pattern `global-monitor.ts` already used. ([#164](https://github.com/Coolhand-Labs/coolhand-node/issues/164))
...
- **A missing (e.g. typo'd) `COOLHAND_PATTERNS_FILE`/`patternsFile` no longer silently disables all monitoring.** This is distinct from the #116 fix below, which covers a patterns file that *exists but is malformed* (wrong shape) — that case already fell back to the built-in default patterns. A *missing* file was handled differently: the synchronous loader used by the constructor logged a warning (suppressed by the library's own default `silent: true`) and left the in-memory pattern list empty, so every subsequent `matchesAPIPatternSync`/`matchesAPIPatternFromURL` call returned `null` — no requests were ever captured, with no visible indication anything was wrong. The not-found branch now falls back to the same built-in default patterns as the malformed-shape case. ([#167](https://github.com/Coolhand-Labs/coolhand-node/issues/167))
...
- **`BaseService`'s `fetch` calls no longer follow redirects.** `validateBaseUrl` restricts a configured `baseUrl` to `https://` (or `http://localhost`/`127.0.0.1`/`[::1]` for local dev) at construction time, but none of the `fetch` call sites (`sendRequest`, `sendMultipart`, `fetchWithHeaders`) passed a `redirect` option, so `fetch`'s default `follow` behavior applied. A `baseUrl` (or a compromised/misconfigured self-hosted deployment behind it) that responded with a 3xx to a different host or to `http://` would have its redirect followed, carrying the `X-API-Key` header and request payload along — defeating the guarantee `validateBaseUrl` exists to provide. All three call sites now pass `redirect: 'error'`, which throws instead of following, aligning `fetch`'s behavior with the Node-<18 `sendWithHTTPS` fallback, which already treated any 3xx as a failure. ([#168](https://github.com/Coolhand-Labs/coolhand-node/issues/168))
- **`RequestMonitoringService`'s `fetch()` interception now correctly matches `fetch(new Request(...))` calls.** `Request.prototype.toString()` isn't overridden by the Fetch API spec (unlike `URL`, which is), so it previously rendered as `"[object Request]"` — no pattern could ever match, so the call silently escaped monitoring, `excludeApiPatterns`, and the self-endpoint exclusion above entirely. A new shared `src/utils/fetch-request-helpers.ts` (`getFetchURL`/`getFetchMethod`/`getFetchHeaders`/`getFetchRequestBody`) is now used by both interception paths, matching the handling `global-monitor.ts` already had for this calling convention.
- **The self-endpoint exclusion above didn't normalize a trailing-dot FQDN.** `new URL(...).hostname` preserves a trailing dot (e.g. `coolhandlabs.com.`, a DNS-valid, semantically identical form some proxies/resolvers canonicalize to) as-is; comparing it against the dotless `baseUrl` hostname would never match, silently bypassing the exclusion and reopening the recursive self-logging bug it exists to prevent. Both `computeSelfEndpoint` and `isSelfEndpointURL` now normalize a trailing dot before comparing.
- **`global-monitor.ts`'s `https.get()`/`http.get()` patches now correctly pass `originalGet` into `interceptRequest`, not `originalRequest`.** This is the `global-monitor.ts` (i.e. `auto-monitor`/`initializeGlobalMonitoring`) half of the same bug class the #161 entry below fixed for `RequestMonitoringService.ts` — and which that entry explicitly flagged as still open here. `interceptRequest` never calls `req.end()` itself; it relies on the caller having done so, which `.get()`'s own wrapper normally handles automatically as part of its contract. Passing `.request` instead of `.get` meant the request object was created but never flushed — no bytes sent, no response, no error, no timeout — so any ordinary `https.get()`/`http.get()` call to a matched host through the auto-monitor path hung forever. Now covered by regression tests exercising both `https.get()` and `http.get()` through `initializeGlobalMonitoring`. ([#202](https://github.com/Coolhand-Labs/coolhand-node/issues/202))
...
- **Host callbacks passed to a patched `http.request`/`https.request` now receive a `PassThrough` tee, not the real `http.IncomingMessage`**, as part of the #115 fix below. `statusCode`, `statusMessage`, `headers`, `rawHeaders`, `httpVersion`, `httpVersionMajor`, `httpVersionMinor`, `method`, `url`, `socket`, `trailers`, `rawTrailers`, `complete`, `aborted`, and `setTimeout()` are all carried over, and `.destroy()` still propagates to abort the real underlying response — but `res instanceof http.IncomingMessage` is now `false`. If your code relies on that `instanceof` check (rather than duck-typing), update it. A host consuming the response via `req.on('response', ...)` instead of the callback argument now receives the same tee, not the raw `res` — see the `req.on('response', ...)` fix in Security below.

### 🔒 Security
- **The SDK's own log-upload endpoint (`baseUrl`/`apiEndpoint`) is now unconditionally excluded from http/https/fetch interception, in both the `Coolhand` class and the auto-monitor (`global-monitor.ts`) paths.** Previously nothing prevented a matched pattern from also matching the SDK's own backend — e.g. a local-dev `baseUrl: 'http://localhost:PORT'` plus a custom `patternsFile` pattern matching `localhost` (for intercepting a local LLM proxy) caused the SDK's own outbound log upload to be captured and re-logged recursively, producing unbounded outbound traffic and a self-inflicted denial of service against whatever `baseUrl` pointed to. The new exclusion matches on hostname + effective port (not substring), so it can't be defeated by a custom pattern and doesn't over-exclude a same-host-different-port service (e.g. a local Ollama proxy). `global-monitor.ts` also gained `excludeApiPatterns` support (plus a `COOLHAND_EXCLUDE_API_PATTERNS` env var for the auto-monitor path) — previously only the `Coolhand` class supported this escape hatch at all.
- **A host using the standard `req.on('response', ...)` calling convention (instead of passing a callback to `.request()`/`.get()`) previously got the raw, un-teed response, reopening the exact async-callback starvation bug the #115 tee fix was meant to close for that calling convention.** The tee was only ever constructed for the callback form. Response delivery is now unified for both calling conventions by intercepting at `req.emit()` itself, so a `req.on('response', ...)` listener gets the same tee-backed protection a callback would.
- **Widened default header redaction to cover `openai-api-key`, `x-goog-api-key`, and `cf-aig-authorization`.** These three provider-specific credential headers were previously redacted only via built-in `api-patterns.json` per-pattern rules, not the unconditional default list — meaning a custom `patternsFile` pattern lacking a `headers` map (or any unmatched/mismatched request) leaked them unredacted, despite the default list's own stated purpose of closing exactly that gap.
- **A second `Coolhand` instance constructed in the same process is no longer silently inert.** `RequestMonitoringService`'s http/https/fetch patches install once per process, permanently bound to whichever instance called `setupMonitoring()` first — a second instance's `excludeApiPatterns`/`baseUrl`/`apiKey` were never consulted, with no indication anything was wrong. Constructing a second instance now emits a clear warning explaining that only one instance's configuration is active per process.
